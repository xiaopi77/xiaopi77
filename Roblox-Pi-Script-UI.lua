...
if theme == 'dark' then
    ...
    TextColor = Color3.fromRGB(0, 255, 255) --蓝色字
    ...
    zyColor = Color3.fromRGB(10, 10, 10)
    beijingColor = Color3.fromRGB(105, 105, 105)
    TextColor = Color3.fromRGB(0, 255, 255) --蓝字
    interfaceTransparency = 0.600 --界面透明
    NameTransparency = 0  --名字透明
end
...
Open.Size = UDim2.new(0, 60, 0, 60) -- 改为60x60
...
spawn(function()
    while Open.Parent do
        for i = 0, 1, 0.01 do
            if not Open.Parent then break end
            local hue = i * 360
            OpenStroke.Color = Color3.fromHSV(hue/360, 1, 1)
            wait(0.05)
        end
    end
end)

-- 为文字添加彩虹渐变动画（使用您提供的代码）
local TextGradient = Instance.new("UIGradient")
TextGradient.Name = "TextRainbowGradient"
...
TextGradient.Parent = Open

-- 使用TweenService创建旋转动画
local tweenInfo = TweenInfo.new(7, Enum.EasingStyle.Linear, Enum.EasingDirection.In, -1)
local tween = services.TweenService:Create(TextGradient, tweenInfo, {Rotation = 360})
tween:Play()
...
-- 主边框彩虹颜色动画（保持不变）
spawn(function()
    while CloudMain.Parent do
        for i = 0, 1, 0.01 do
            if not CloudMain.Parent then break end
            local hue = (i * 360 + 180) % 360  -- 与按钮颜色相差180度
    ...
    function window.Tab(window, name, icon)
        ...
        function tab.section(tab, name, TabVal)
            ...
            function section.Toggle(section, text, flag, enabled, callback)
                ...
                ToggleDisableC.CornerRadius = UDim.new(0, 8)
